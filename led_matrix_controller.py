...
import sys
import httpx
from matrix_setup import draw_circle, draw_diamond
...
if platform.system() == "Linux":
    ...
    def render_train_line(section: int, train_data: dict):
        ...
        if line == 'F' and is_express:
            draw_diamond(matrix, graphics, circle_x, circle_y, circle_radius, circle_color)
        else:
            draw_circle(matrix, graphics, circle_x, circle_y, circle_radius, circle_color)
    ...
    while True:
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get("http://mother.local:4599/trains/fg-northbound-next", timeout=5.0)
